...
from pyparsing import CaselessKeyword, Forward, Word, Regex, alphanums, \
    delimitedList, Suppress, Optional, Group, OneOrMore,ZeroOrMore,alphas
...
class DdlParseColumn(DdlParseTableColumnBase):
    ...
    def constraint(self):
        ...
        if self._not_null:
            constraint_arr.append("FOREIGN KEY" if self._fk else "NOT NULL")
        if self._unique:
    ...
    def constraint(self, constraint):
        ...
        if type(constraint) is str:
            ...
            self._fk = False if self._constraint is None or not re.search("FOREIGN KEY", self._constraint) else True
        ...
        self._fk = True if re.search("FOREIGN", constraints['key'], re.IGNORECASE) else False
...
class DdlParse(DdlParseBase):
    ...
    _empty_line=Suppress("--")
    _pattern=Suppress(OneOrMore("/*!"+ Regex(r".+")))


    _grammer_before_query=OneOrMore(_COMMENT)+ZeroOrMore(_pattern)+_COMMENT +Suppress("--")+Suppress(OneOrMore(Word(alphanums)))+\
    Suppress(Optional(("`")+Word(alphas)+("`")+(";")))+ZeroOrMore(_pattern)
    ...
    _DDL_PARSE_EXPR << OneOrMore(_grammer_before_query+ _CREATE_TABLE_STATEMENT|_COMMENT )
